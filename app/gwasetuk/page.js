...
export default function GwasetukPage() {
    ...
    const generatePrompt = (student, selectedActivities, targetChars, individualActivity = "", model = "", searchContext = "") => {
        const gradeDescriptions = {
            ...
            C: "C(보완 필요) - 활동의 일부 기준점에 아직 충분히 도달하지 못한 부분을 드러내고, 기초 이해와 표현의 구체성을 단계적으로 보완할 필요가 있음을 교사다운 문장으로 서술",
...
- C 전용 권장 표현: 일부 기준점에 아직 충분히 도달하지 못한 부분이 있어 단계적인 보완이 필요함, 기초 개념 이해를 더 보완할 필요가 있음, 결과 완성도와 표현의 구체성을 높이기 위한 연습이 요구됨
...
- C 활동에는 일부 기준점에 아직 충분히 도달하지 못한 부분, 기초 이해의 제한, 단계적인 보완이 필요한 지점을 분명히 쓰되 못함·소극적·미흡함 같은 비판적 낙인 표현은 쓰지 않음
...
- A는 최고 수준의 성취, B는 안정적 수행, C는 부분 보완, D는 많은 보완, E는 매우 많은 보완으로 서술 기조를 구분함
